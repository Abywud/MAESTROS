...
playBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // Check if data is returning the expected properties
    if (!data || !data.winner) {
      resultDiv.innerText = `⚠️ Unexpected response. Please try again.`;
      return;
    }
    
    resultDiv.innerText = data.message;
    
    // Log the winner and check if it's correct
    console.log(`Winner from API: ${data.winner}`);
